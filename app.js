...
const SOUNDS = {
  ...
  'Crowd Cheer':    [[200,0,.08,'sawtooth',.2],[300,.05,.15,'sawtooth',.25],[250,.12,.3,'sawtooth',.2],[350,.25,.5,'sawtooth',.15]],
  'Explosion':      [[80,0,.12,'square',.5],[60,.08,.25,'square',.45],[100,.2,.4,'sawtooth',.3]],
...
const SoundManager = {
  ...
  addGiftCustom() {
    ...
    const sound = el('gs-sound-select').value;
    if (!name) return;
    this.customGifts[name.toLowerCase()] = sound;
    el('gs-name').value = '';
    this.save(); this.renderGiftCustomList();
  },
  ...
  renderGiftCustomList() {
    el('gift-custom-list').innerHTML = Object.entries(this.customGifts).map(([n,s]) =>
      `<span class="tag" onclick="SoundManager.removeGiftCustom('${n}')" title="Remove">${n} → ${s} ✕</span>`
    ).join('');
  },
  ...
  buildAllPickers() {
    ...
    // Gift custom sound dropdown
    const gsSel = el('gs-sound-select');
    if (gsSel) gsSel.innerHTML = SOUND_NAMES.map(n=>`<option>${n}</option>`).join('');
  },
...
const VoicePicker = {
  voices: [],
  load() {
    this.voices = speechSynthesis.getVoices().filter(v => v.lang && v.lang.startsWith('en'));
  ...
  preview() {
    ...
    const utt = new SpeechSynthesisUtterance('Hi! This is how I will sound during your stream. Welcome everyone!');
    utt.voice = voice;
    utt.rate = S.speechRate;
    utt.volume = S.voiceVolume;
...
const Events = {
  ...
  gift(username, giftName, coins, emoji, repeatCount) {
    ...
    const sound = SoundManager.getForGift(giftName, size);
...
const TikTok = {
  guideStep:0,
  guideSteps:[
    "Step 1 of 6. Open a new browser tab and go to tiktok dot com. Log in with your TikTok account if you are not already. Come back here and press Next when you are on TikTok.",
    "Step 2 of 6. While on the TikTok website, press the F 12 key on your keyboard. This opens the developer tools panel on the side or bottom of the screen. Press Next when it has opened.",
    "Step 3 of 6. Inside the developer tools, look along the top for a tab called Application and click it. It might be hidden inside a dropdown — look for a double arrow button if you cannot see it. Press Next when you are on the Application tab.",
    "Step 4 of 6. On the left side of the Application panel, look for the word Cookies with an arrow next to it. Click the arrow to expand it, then click on the item underneath that says tiktok dot com. Press Next.",
    "Step 5 of 6. You will now see a list of cookies. Find the one called sessionid — all one word, all lowercase. Click on it. On the right side you will see a long string of letters and numbers — that is your session ID. Select all of it and copy it. Press Next when you have copied it.",
    "Step 6 of 6. Come back to this window and paste your session ID into the Session ID box below using Control V on your keyboard. Then press Connect. You are done — the app will remember this forever so you only ever have to do this once."
  ],
  toggleConnect(){
  ...
  showModal(){
    ...
    el('tiktok-username-input').onkeydown=e=>{if(e.key==='Enter')el('tiktok-session-input').focus();};
  ...
  connect(){
    ...
    if(!username){el('modal-error').textContent='Please enter your TikTok username.';return;}
    if(!sessionId){el('modal-error').textContent='Session ID is required. Press the help button above to hear how to get it.';Speech.speak('Session ID is needed. Press the help button to hear the step by step instructions.',{rate:.95,volume:1.0});return;}
    el('modal-error').textContent='Connecting...';
    localStorage.setItem('tla-sessionid',sessionId);
    socket.emit('connect-tiktok',{username,sessionId});
  },
  closeModal(){el('tiktok-modal').classList.add('hidden');Speech.cancel();},
  startGuide(){this.guideStep=0;this.readStep();},
  readStep(){
    const text=this.guideSteps[this.guideStep]||'';
    el('guide-text').textContent=text;
    el('guide-step-num').textContent=`Step ${this.guideStep+1} of ${this.guideSteps.length}`;
    Speech.cancel();
    Speech.speak(text,{rate:.92,volume:1.0});
  },
